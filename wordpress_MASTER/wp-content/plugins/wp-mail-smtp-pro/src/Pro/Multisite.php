...
class Multisite {
	...
	public function init() {
		...
		// Add the plugin admin pages for WP MS and remove unneeded menu items.
	...
	public function maybe_remove_plugin_admin_pages_from_child_sites() {
		...
		// Use fresh option generation, to maybe pull in the main network site options.
		$multisite_enabled = ( new Options() )->get( 'general', 'network_wide' );

		if ( ! empty( $multisite_enabled ) && $multisite_enabled === true ) {
			add_action(
				'admin_menu',
				function () {
